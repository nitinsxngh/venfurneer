...
interface OrderListProps {
    ...
    onPaymentStatusUpdate: (orderId: string, paymentStatus: Order['paymentStatus']) => void;
}

const OrderList = ({ orders, loading, onStatusUpdate, onPaymentStatusUpdate }: OrderListProps) => {
                    ...
                    <table>
                        ...
                        <tbody>
                            {filteredOrders.map((order) => (
                                <tr key={order.id}>
                                    <td>
                                        <strong className="admin-table__order-number">{order.orderNumber}</strong>
                                    </td>
                                    <td>
                                        <div className="admin-table__customer-info">
                                            <div className="admin-table__customer-name">{order.customer.name}</div>
                                            <div className="admin-table__customer-email">{order.customer.email}</div>
                                            {order.customer.phone && (
                                                <div className="admin-table__customer-phone">{order.customer.phone}</div>
                                            )}
                                        </div>
                                    </td>
                                    <td>
                                        <div className="admin-table__order-items">
                                            {order.items.map((item, index) => (
                                                <div key={index} className="admin-table__order-item">
                                                    ...
                                                    <span className="admin-table__item-quantity">x{item.quantity}</span>
                                                    <span className="admin-table__item-price">₹{item.price}</span>
                                                </div>
                                            ))}
                                        </div>
                                    </td>
                                    <td>
                                        <div className="admin-table__order-total">
                                            <div className="admin-table__total-amount">₹{order.total.toLocaleString()}</div>
                                    ...
                                    <td>
                                        <select
                                            ...
                                            onChange={(e) => onPaymentStatusUpdate(order.id, e.target.value as Order['paymentStatus'])}
